...
# How are the statistics affected if TGCT is treated as an outlier?
print(summary(lm(longDF$AvgCNA[!rownames(longDF) == "TGCT"] ~ longDF$giiCor[!rownames(longDF) == "TGCT"])))
...
# How are the statistics affected if TGCT is treated as an outlier?
print(summary(lm(longDF$VarExpr[!rownames(longDF) == "TGCT"] ~ longDF$VarMeth[!rownames(longDF) == "TGCT"])))
...
##################################################
## Mean meth variance vs. mean CN variance scatterplot

# Fit linear model
fit <- lm(longDF$VarCNA ~ longDF$VarMeth)

# How are the statistics affected if TGCT is treated as an outlier?
print(summary(lm(longDF$VarCNA[!rownames(longDF) == "TGCT"] ~ longDF$VarMeth[!rownames(longDF) == "TGCT"])))
...
# Start figure
pdf(file = paste0("./Output/Figures/CN-variance_vs_Meth-variance_",
                  w_BP_short,
                  "kWindow.pdf"),
    width = 4,
    height = 3,
...
# Make plot
plot(longDF$VarCNA ~ longDF$VarMeth,
     bg = longDF$TumorType,
     col = "black",
     pch = pointTypes,
     ylim = c(0, 0.4),
     ...
     bty = "L",
     xlab = "Mean MethSig variance",
     main = "CN variance ~ MethSig variance",
     font.main = 1,
     ylab = "Mean copy number variance")
...
# Add annotation
text(x = 0.06, y = 0.36, pos = 4, labels = bquote(italic(P)~.(p)))
