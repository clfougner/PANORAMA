...
foreach(tt = tumorTypes) %dopar% {
  ...
  overlappingGenes <- overlappingGenes[overlappingGenes %in% rownames(copyNumber)]
    ...
    # Choose samples (this is unnecessary, but keeping in case removing it breaks anything)
    currentSamplesNums <- 1:ncol(currentMethylation[[1]])
    ...
    for(j in 1:length(overlappingGenes)){
      ...
      # Bind the methylation, CN and gene expression data frames together
      ...
      if(nrow(subDF) < 0.8 * nrow(subDF_meth)){next}
    ...
    write.table(allDF,
                file = paste0("./Output/AllxSamplesxModels_", w_BP_short, "kWindow_", tt, ".txt"),
                sep = "\t",
                quote = FALSE)
}
